...
import com.example.kueski.feature_flag.impl.KueskiRemoteConfigValue
...
class RemoteConfigManagerImpl @Inject constructor(
    ...
    init {
        ...
        FirebaseInstanceId.getInstance().token?.let { logger.i(FIREBASE_TOKEN, it) }
    }
    ...
    override fun getValue(key: String): RemoteConfigValue {
        return KueskiRemoteConfigValue(remoteConfig.get().getValue(key), gson)
    }
